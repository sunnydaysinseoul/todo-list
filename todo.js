...
function deleteToDo(event) {
  ...
  const cleanToDos = toDos.filter(function filterFn(toDo) {
    //Array의 foreach처럼 하나씩 실행하는 함수를 만들 것
...
function paintDones(text){
  ...
  delBtn.addEventListener("click", deleteDone); //delBtn버튼을 click하는 event가 일어날 때 deleteToDo()함수를 실행하기.
  li.appendChild(delBtn);
  li.appendChild(span); //parent(li)에 span값을 넣는 것
  li.id = newId; //입력시마다 <li id="n"> n이 1씩 증가함

  toDoDone.appendChild(li); //delBtn,span(text)을 toDoList 자리에 추가하기
  ...
  saveDones(); //지금까지의 toDos를 localStorage에 저장하기
}
...
function saveDones() {
  localStorage.setItem(DONE_LS, JSON.stringify(doneToDos));
}
function saveToDos() {
...
function paintToDo(text) {
  ...
  checkBtn.addEventListener("click", getDones); //checkBtn눌렀을 때 실행할 함수1
  checkBtn.addEventListener("click", deleteToDo); //checkBtn눌렀을 때 실행할 함수2
  li.appendChild(delBtn);
  li.appendChild(span); //parent(li)에 span값을 넣는 것
  ...
  const toDoObj = {
    //array안에 넣어 줄 Object(dictionary)생성
